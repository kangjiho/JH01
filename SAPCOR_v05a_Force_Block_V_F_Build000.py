...
#{
  ...
  pass #[Modified]  BN = Core['Array'][K][L]
  BN = Core['BTNsKL'][K][L]
  BT = BlockTypes[BN]
  A = BT['a']
  H = BT['h']
  ...
  d_mu = BT['d_mu']
  xi_F_cr = BT['xi_F_cr']
  pass #[Deleted]  Fixed = BT['Fixed']
  pass # endregion

  # region Get Block Prop of (K,L-1)
  # Subscript, m, means 'Minus 1'
  if L!=1:
    pass #[Modified] BN = Core['Array'][K][L-1]
    BN = Core['BTNsKL'][K][L-1]
    BT = BlockTypes[BN]
    pass #[Deleted] Am = BT['a']
    Hm = BT['h']
    Mm = BT['M']
    Im = BT['I']
    pass #[Deleted] Kvm = BT['Kv']
    pass #[Deleted] Cvm = BT['Cv']
    pass #[Deleted] Fixedm = BT['Fixed']
  else:
  ...
  if L!=1:
    ...
    #}
  pass # endregion Get current state of (K,L-1)

  pass # endregion INITIALIZATION

  #############################################################################
  # region VERTICAL FORCES
##############################################################################
#{  VERTICAL FORCES                                                          #
#                                                                            #
#                                                                            #
...
# ['Block_V'] is already checked in VectorField()
  ...
  # region Spring Contraction between (K,L-1)~(K,L)
  if L!=1:
...
#    if M_VRm > 0. : M_VRm = 0.
  ...
  pass # endregion Spring Contraction between (K,L-1)~(K,L)
  ...
  pass # endregion Assemble Forces
  ...
  if POST==True:
    ...
    try: fo = open(Filename,'r')
      ...
      fo.write(temp)
      fo.close()
    pass # endregion HEADER
    
    # region F_VLk, F_VLc, F_VRk, F_VRc
    F_VLk = F_VLc = 0.
    F_VRk = F_VRc = 0.
    if Gamma_L>0.:
      F_VLk = Kv * Gamma_L
    ...
    if Gamma_R>0.:
      F_VRk = Kv * Gamma_R
      F_VRc = Cv * DGamma_R
    pass # endregion F_VLk, F_VLc, F_VRk, F_VRc
...
#}############################################################################
  pass # endregion VERTICAL FORCES

##############################################################################
# region  FRICTION FORCES

  pass #[Modified v04f] if Core['ApplyForces']['Block_F']==True:
    ...
    if F_VL>0: 
      ...
      if xi_L!=0:
        ...
        if abs(xi_L) <= xi_F_cr:
          ...
          pass #[Modified v04i] F_FL = - (+(xi_L>0) or -(xi_L<0)) * mu * F_VL
          F_FL = - cmp(xi_L,0) * mu * F_VL
        pass # endregion Friction Force: F_FL
          
        # region Other Forces & Moments: F_FLm, M_FL, M_FLm
        F_FLm = -F_FL
        M_FL    = -F_FL  * ( H  * cos( R  ) - A * sin( R  ) )
        if L!=1:
          M_FLm =  F_FLm * ( Hm * cos( Rm ) + A * sin( Rm ) )
        pass # endregion Other Forces & Moments: F_FLm, M_FL, M_FLm
        
      pass # Check Point: xi==0 -> Do nothing
        
    pass # Check Point: F_VL<=0 -> Do nothing
...
#v04i     F_FR = - (+(xi_R>0) or -(xi_R<0)) * mu * F_VR
          F_FR = - cmp(xi_R,0) * mu * F_VR
    ...
    pass # Check Point: F_VR<=0 -> Do nothing
    
    
    pass # endregion RIGHT

    #===========================================================================
    # region Assemble Accel
    AccelF  =  (F_FL  + F_FR ) / M
    AccelM  =  (M_FL  + M_FR ) / I
    ...
    if L!=1: # Normal Blocks
      ...
      pass #[Modified v04f] IndexWm = Core['Index'][K][L-1]*6
      IndexWm = Core['Index'][1][0]*6
      Accel[IndexWm+1] += AccelFm # DDU
      pass # No moment on CSB
  
    else: pass # if Base: Do nothing
    pass # endregion Assemble Accel
