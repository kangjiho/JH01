...
# -------------------------------------------------------------------
# FILE IO
fo=file(FN_OUT+'.csv','w')
fo_shape=file(FN_OUT+'_shape.csv','w')
Verbose('//INIT//')
# -------------------------------------------------------------------
...
#    print '  t[0]=%f, t[-1]=%f, len(t)=%d'%(t[0],t[-1],len(t))
    Solution,Status = odeint(VectorField, StartValues, t, args=(Core,VERBOSE),
              atol=ABS_ERR, rtol=REL_ERR, full_output=True)
    ...
    # If odeint failed, Half SectionTime and rerun
    SectionTime/=2.
    print '    Half the SectionTime = %f'%SectionTime
    if SectionTime<MIN_SectionTime:
      print '='*70
      print 'ERR: SectionTime(%e) < MIN_SectionTime(%e)'%(SectionTime,MIN_SectionTime)
      print 'STOP'
      print '='*70
      sys.exit(-1)
