...
def Force_Block_H (w, t, Core, K, L, Accel, POST=False, FO_DIR=FO_DIR):
  ...
  if K < Core['M']:
    ...
    if '/ForceH/' in VERBOSE:
      ...
      if 'Right' in ContactConfig:
        SignK = 1
      elif 'Left' in ContactConfig:
        SignK = -1
      else:
        ERROR('Error: ContactConfig("%s") was invalid.' % ContactConfig)
